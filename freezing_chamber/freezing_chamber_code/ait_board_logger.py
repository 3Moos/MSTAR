...
pico5 = Serial('COM', 115200, timeout=2)
pico6 = Serial('COM', 115200, timeout=2)
pico7 = Serial('COM', 115200, timeout=2)
pico8 = Serial('COM', 115200, timeout=2)
picos = [pico1, pico2, pico3, pico4, pico5, pico6, pico7, pico8]

p5com = # add COM port for board 5
p6com = # add COM port for board 6
p7com = # add COM port for board 7
p8com = # add COM port for board 8

b3com = # add COM port for board 3
b4com = # add COM port for board 4



power1 = Serial('COM27', 115200, timeout=2)
power2 = Serial('COM28', 115200, timeout=2)
power3 = Serial(f'COM{b3com}', 115200, timeout=2)
power4 = Serial(f'COM{b4com}', 115200, timeout=2)
...
board_3 = f"board3 - COM{b3com}"
board_4 = f"board4 - COM{b4com}"

temp_boards = file_name + " " + testing_temperature + f"Temp Boards - COM15-COM18-COM16-COM14-{p5com}-COM{p6com}-COM{p7com}-COM{p8com}"
board_1 = board_1 + " " + testing_temperature
board_2 = board_2 + " " + testing_temperature
board_3 = board_3 + " " + testing_temperature
board_4 = board_4 + " " + testing_temperature
...
board3_name = os.path.join(folder_name, board_3 + " " + timestamp + ".csv")
board4_name = os.path.join(folder_name, board_4 + " " + timestamp + ".csv")
...
name4 = os.path.join(folder_name, " power 3 " + timestamp + ".csv")
name5 = os.path.join(folder_name, " power 4 " + timestamp + ".csv
...
with open(name1, 'w', newline='') as csvfile:
    ...
    csvwriter.writerow(['Timestamp', 'T1 - COM15', 'T2 - COM15', 'T3 - COM18', 'T4 - COM18', 'T5 - COM16', 'T6 - COM16', 'T7 - COM14', 'T8 - COM14', f'T9 - COM{p5com}', f'T10 - COM{p5com}', f'T11 - COM{p6com}', f'T12 - COM{p6com}', f'T13 - COM{p7com}', f'T14 - COM{p7com}', f'T15 - COM{p8com}', f'T16 - COM{p8com}'])
...
with open(name4, 'w', newline='') as csvfile:
    csvwriter = csv.writer(csvfile)
    # Write the header to the CSV file
    csvwriter.writerow(['Timestamp', "mA 1", "V1", "mA 2", "V2", "mA 3", "V3", "mA 4", "V4", "mA 5", "V5", "mA 6", "V6", "mA 7", "V7", "mA 8", "V8"])

with open(name5, 'w', newline='') as csvfile:
    csvwriter = csv.writer(csvfile)
    # Write the header to the CSV file
    csvwriter.writerow(['Timestamp', "mA 1", "V1", "mA 2", "V2", "mA 3", "V3", "mA 4", "V4", "mA 5", "V5", "mA 6", "V6", "mA 7", "V7", "mA 8", "V8"])
...
while True:
    try:
        ...
        power3.reset_input_buffer()
        power3.write(b'get_readings\r\n')
        power_line3 = power3.readline()
        power_line3 = power_line3.decode().strip()

        power4.reset_input_buffer()
        power4.write(b'get_readings\r\n')
        power_line4 = power4.readline()
        power_line4 = power_line4.decode().strip()
        ...
        with open(name4, 'a+', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            # Write the timestamp and the line to the CSV file
            #timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            
            data = power_line3.split(",")
            data.insert(0, timestamp)
            csvwriter.writerow(data)

        with open(name5, 'a+', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            # Write the timestamp and the line to the CSV file
            #timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            
            data = power_line4.split(",")
            data.insert(0, timestamp)
            csvwriter.writerow(data)
